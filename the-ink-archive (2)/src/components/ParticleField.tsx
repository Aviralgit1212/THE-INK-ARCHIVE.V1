...
export default function ParticleField() {
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      if (currentRoute !== '/' && localMode !== 'ambient') {
        localMode = 'ambient';
      ...
      const isCompressedPressed = (currentRoute === '/') && (mouse.isDown || (hand.active && hand.isFist));
      ...
      if (localMode === 'ambient') {
        ...
        if (!isCompressedPressed) {
          ...
          progressFist += 0.007; // ~3.5 seconds of compression effort
          setFistProgress(Math.min(progressFist, 1));
        ...
        titleTimer += 1;
        const holdThreshold = entryMethodRef.current === 'button' ? 180 : 330;
        if (titleTimer > holdThreshold) {
          localMode = 'dissolve';
        ...
        dissolveTimer += 1;
        if (dissolveTimer > 120) { // 2.0 seconds dissolve transition
          navigateTo('/library');
          localMode = 'ambient';
          setSystemMode('ambient');
          progressFist = 0;
      ...
      } else if (currentRoute === '/library/[collection]') {
      ...
      } else if (currentRoute.startsWith('/read')) {
              ...
              <div 
                ...
                style={{ 
                  opacity: systemMode === 'dissolve' 
                    ...
                    : Math.max(0, Math.min(1, (titleTimerRef.current - 30) / 60))
                }}
              >
